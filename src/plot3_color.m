function fig_hand = plot3_color(x,y,z,c,clim,mark)
...
% PLOT3_COLOR color coded 3D scatterplot
%    fig_hand = plot3_color(x,y,z,c,clim,mark)
% Input
%    x, y, z    location vectors%              
...
%
% Generate a 3D scatter plot of pmat=(x,y,z) using the values in the vector c to
...
% Example
%    [x,y,z] = peaks(101);
%    plot3k(x, y, z,gradient(z),[-0.5 0.5],{'o',2});
...
pmat(:,4) = min(max(round((c-min(cmin,cmax))*(clen-1)/abs(crange)),1),clen);

% sort by color map index
pmat = sortrows(pmat,4);

% build vector of last indices corresponding to each unique color index (last point for each color)
cind_last = [find(diff(pmat(:,4))>0); length(pmat)];
...
for i_cind = 1:length(cind_last) % loop over each non-empty color group
   plot3(pmat(cind_first:cind_last(i_cind),1), ... % call plot3 once for each color group
         pmat(cind_first:cind_last(i_cind),2), ...
         pmat(cind_first:cind_last(i_cind),3), ...
         mark_type,           ...
         'MarkerSize',mark_size, ...
         'MarkerEdgeColor',cmap(pmat(cind_first,4),:), ... % same marker color from cmap
         'MarkerFaceColor',cmap(pmat(cind_first,4),:) ); % for all points in group
